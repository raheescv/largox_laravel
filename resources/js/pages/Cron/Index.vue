    ...
    <div class="space-y-6">
        ...
        <Card>
            <CardHeader>
                ...
                <CardDescription>Full contents of crontab -u {{ form.user }} -l</CardDescription>
            </CardHeader>
            <CardContent class="space-y-4">
                <div class="flex items-end gap-3">
                    <div class="grid gap-2">
                        <Label for="user">User</Label>
                        <Input id="user" v-model="form.user" class="w-36" @keydown.enter.prevent="switchUser" />
                    </div>
                ...
                <form @submit.prevent="submit" class="space-y-4">
                    ...
                    />
                    <p v-if="form.errors.contents" class="text-sm text-destructive">{{ form.errors.contents }}</p>
                    <p v-if="form.errors.agent" class="text-sm text-destructive">{{ form.errors.agent }}</p>
                    <Button type="submit" :disabled="form.processing">Save crontab</Button>
                </form>
            </CardContent>
        </Card>
    </div>

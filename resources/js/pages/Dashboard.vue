        ...
        <template v-else>
            ...
            >
                ...
                <div class="flex items-center gap-2">
                    ...
                    />
                    <span class="text-sm">{{
                        reachable ? 'Agent online' : 'Agent unreachable'
                    }}</span>
                </div>
            </div>
            ...
            <div v-if="metrics" class="grid gap-4 md:grid-cols-3">
                ...
                <div class="rounded-xl border p-4">
                    ...
                    <div class="mt-2 text-xs text-muted-foreground">
                        {{ metrics.cpu_count }} cores · {{ metrics.os }}/{{
                            metrics.arch
                        }}
                    </div>
                </div>
                ...
                <div class="rounded-xl border p-4">
                    ...
                    <div
                        class="mt-3 h-2 w-full overflow-hidden rounded bg-muted"
                    >
                        <div
                            class="h-full transition-all"
                            :class="barColor(metrics.memory.used_pct)"
                            :style="{
                                width: metrics.memory.used_pct.toFixed(1) + '%',
                            }"
                        />
                    </div>
                    <div class="mt-1 text-xs text-muted-foreground">
                        {{ metrics.memory.used_pct.toFixed(1) }}% used
                    </div>
                </div>
                ...
                <div class="space-y-3">
                    <div v-for="d in metrics.disk" :key="d.mount">
                        <div class="flex justify-between text-sm">
                            ...
                            <span class="text-muted-foreground">
                                {{ d.used_gb.toFixed(1) }} /
                                {{ d.total_gb.toFixed(1) }} GB ({{
                                    d.used_pct.toFixed(1)
                                }}%)
                            </span>
                        </div>
                        <div
                            class="mt-1 h-2 w-full overflow-hidden rounded bg-muted"
                        >
                            <div
                                class="h-full transition-all"
                                ...
                                :style="{
                                    width:
                                        Math.min(100, d.used_pct).toFixed(1) +
                                        '%',
                                }"
                            />
                        </div>
